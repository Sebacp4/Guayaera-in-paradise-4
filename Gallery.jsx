...
const GalleryItem = ({ src, onClick, alt, cta }) => (
  <div
...
const Column = ({ images, direction, hiddenClass, onImageClick, alt, cta }) => (
  <div className={`flex-1 h-full overflow-hidden relative ${hiddenClass}`}>
    <div className={`flex flex-col w-full h-max animate-scroll-${direction}`}>
      <div className="flex flex-col gap-4 pb-4 w-full">
        {images.map((src, i) => (
          <GalleryItem key={`g1-${i}`} src={src} onClick={onImageClick} alt={alt} cta={cta} />
        ))}
      </div>
      <div className="flex flex-col gap-4 pb-4 w-full">
        {images.map((src, i) => (
          <GalleryItem key={`g2-${i}`} src={src} onClick={onImageClick} alt={alt} cta={cta} />
        ))}
      </div>
    </div>
  </div>
      ...
      <section className="pt-36 pb-16 bg-[#000000] overflow-hidden relative border-b border-[#FDFAF5]/10">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center fade-in-up relative z-10 visible">
          ...
          <h1 className="text-6xl md:text-8xl text-[#FDFAF5] font-anton tracking-tighter uppercase mb-6 leading-none">
            {t('gallery.titlePrefix')} <span className="text-[#EB459A]">{t('gallery.titleHighlight')}</span>
          </h1>
      ...
      <section className="relative w-full overflow-hidden bg-[#000000] flex-grow pt-4 pb-4">
        ...
        <div className="max-w-[1600px] mx-auto w-full px-4 h-[80vh] min-h-[600px] max-h-[900px] relative">
          <div className="flex gap-4 h-full w-full justify-center">
            <Column images={col1} direction="up" hiddenClass="" onImageClick={setSelectedImage} alt={t('gallery.momentAlt')} cta={t('gallery.viewMoment')} />
            <Column images={col2} direction="down" hiddenClass="" onImageClick={setSelectedImage} alt={t('gallery.momentAlt')} cta={t('gallery.viewMoment')} />
            <Column images={col3} direction="up" hiddenClass="hidden md:block" onImageClick={setSelectedImage} alt={t('gallery.momentAlt')} cta={t('gallery.viewMoment')} />
            <Column images={col4} direction="down" hiddenClass="hidden lg:block" onImageClick={setSelectedImage} alt={t('gallery.momentAlt')} cta={t('gallery.viewMoment')} />
          </div>
        </div>
      </section>
