...
export default function Raffle() {
  ...
  const steps = t('raffle.process.steps', { returnObjects: true });
  const receiveItems = t('raffle.receive.items', { returnObjects: true });
  const firstPrizeItems = t('raffle.prizes.firstPrizeItems', { returnObjects: true });
  const secondPrizeItems = t('raffle.prizes.secondPrizeItems', { returnObjects: true });
  const thirdPrizeItems = t('raffle.prizes.thirdPrizeItems', { returnObjects: true });
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (!response.ok || !data.url) {
        throw new Error(data.error || t('raffle.errors.checkoutSession'));
      }

      window.location.href = data.url;
    } catch (error) {
      console.error('Unable to start Stripe Checkout:', error);
      setCheckoutError(t('raffle.hero.checkoutError'));
      setIsCheckoutLoading(false);
    }
  };

  const stepMeta = [
    { num: '1', icon: 'solar:cart-large-bold-duotone', color: '#EB459A' },
    { num: '2', icon: 'solar:letter-bold-duotone', color: '#01C9CF' },
    { num: '3', icon: 'solar:qr-code-bold-duotone', color: '#EB7A4B' },
    { num: '4', icon: 'solar:medal-ribbon-star-bold-duotone', color: '#EB459A' },
  ];
    ...
    <div className="flex flex-col flex-grow bg-[#000000] animate-page-enter relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <div className="text-center lg:text-left fade-in-up visible">
              ...
              >
                ...
                {isCheckoutLoading ? t('raffle.hero.redirecting') : t('raffle.hero.button')}
              </button>
              {checkoutError && (
              ...
              <div className="relative z-10 bg-[#FDFAF5] rounded-3xl p-8 md:p-10 shadow-2xl border border-[#000000]/5 overflow-hidden group hover:shadow-[0_0_40px_rgba(1,201,207,0.3)] transition-all duration-500">
                ...
                <div className="border-t border-b border-[#000000]/10 py-6 mb-6 relative z-10">
                  <div className="font-bebas text-sm tracking-widest uppercase text-[#000000]/50 mb-1">
                    {t('raffle.hero.reservationLabel')}
                  </div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="text-center mb-16 fade-in-up">
            <h2 className="font-bebas text-[#01C9CF] text-3xl tracking-tight uppercase mb-2">
              {t('raffle.process.eyebrow')}
            </h2>
            <h3 className="font-anton text-5xl md:text-7xl tracking-tighter uppercase leading-[0.9] text-[#000000]">
              {t('raffle.process.titlePrefix')} <span className="text-[#EB7A4B]">{t('raffle.process.titleHighlight')}</span>
            </h3>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {steps.map((step, i) => (
              ...
              >
                <div className="absolute top-0 left-0 w-full h-1 transition-all duration-300 group-hover:h-2" style={{ backgroundColor: stepMeta[i].color }}></div>

                <div className="flex items-center justify-between w-full mb-6">
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ backgroundColor: `${stepMeta[i].color}1A`, color: stepMeta[i].color }}>
                    <iconify-icon icon={stepMeta[i].icon} width="32" height="32"></iconify-icon>
                  </div>
                  <span className="font-anton text-6xl leading-none opacity-10" style={{ color: stepMeta[i].color }}>
                    {stepMeta[i].num}
                  </span>
                </div>
        ...
        <div className="max-w-5xl mx-auto px-6 relative z-10">
          <div className="text-center mb-16 fade-in-up">
            <h2 className="font-bebas text-[#EB459A] text-3xl tracking-tight uppercase mb-2">
              {t('raffle.receive.eyebrow')}
            </h2>
            <h3 className="font-anton text-5xl md:text-7xl tracking-tighter uppercase leading-[0.9]">
              {t('raffle.receive.titlePrefix')} <span className="text-[#01C9CF]">{t('raffle.receive.titleHighlight')}</span>
            </h3>
          </div>
            ...
            {receiveItems.map((item, i) => (
              ...
              >
                <div className="w-14 h-14 rounded-2xl bg-[#01C9CF]/10 flex items-center justify-center text-[#01C9CF] shrink-0 group-hover:bg-[#01C9CF] group-hover:text-[#000000] transition-all duration-300">
                  <iconify-icon icon={receiveIcons[i]} width="32" height="32"></iconify-icon>
                </div>
              ...
              <div className="bg-white rounded-3xl overflow-hidden shadow-2xl border border-[#000000]/10 flex flex-col h-full card-hover group relative">
                <div className="absolute top-6 left-6 z-20">
                  <span className="bg-[#EB459A] text-white font-bebas text-2xl px-6 py-2 uppercase tracking-wider rounded-lg shadow-xl border border-white/20">
                    {t('raffle.prizes.firstPrizeLabel')}
                  </span>
                </div>
                <div className="aspect-[16/9] w-full overflow-hidden relative">
                  <img
                    ...
                    alt={t('raffle.prizes.firstPrizeTitle')}
                  ...
                  <div className="absolute bottom-6 left-8 right-8 text-white z-10">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="font-bebas text-3xl uppercase tracking-wide drop-shadow-md">{t('raffle.prizes.vipExperience')}</span>
                    </div>
                  </div>
                </div>
                <div className="p-8 md:p-10">
                  <h4 className="font-anton text-4xl md:text-5xl text-[#000000] uppercase tracking-tighter mb-6 leading-tight">
                    {t('raffle.prizes.firstPrizeTitle')}
                  </h4>
                  <ul className="space-y-4 text-xl font-medium text-[#000000]/70 mb-8">
                    {firstPrizeItems.map((item) => (
                      <li key={item} className="flex items-start gap-4">
                        <iconify-icon icon={item === firstPrizeItems[0] ? 'solar:calendar-date-bold' : item === firstPrizeItems[1] ? 'solar:medal-ribbon-bold-duotone' : item === firstPrizeItems[2] ? 'icon-park-twotone:parking' : 'solar:tea-cup-bold-duotone'} className={item === firstPrizeItems[0] ? 'text-[#EB459A] shrink-0 mt-1' : item === firstPrizeItems[1] ? 'text-[#01C9CF] shrink-0 mt-1' : item === firstPrizeItems[2] ? 'text-[#EB7A4B] shrink-0 mt-1' : 'text-[#01C9CF] shrink-0 mt-1'} width="24" height="24"></iconify-icon>
                        <span>{item}</span>
                      </li>
                    ))}
                  </ul>
                  <div className="pt-6 border-t border-[#000000]/10 flex items-center justify-between">
                    <div className="bg-[#000000] text-white p-4 rounded-xl shadow-lg -rotate-3 border border-white/10 group-hover:rotate-0 transition-transform">
                      <div className="font-bebas text-lg tracking-widest uppercase mb-1">{t('raffle.prizes.vipPassLabel')}</div>
                      <div className="font-anton text-2xl text-[#01C9CF]">{t('raffle.prizes.vipPassValue')}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl overflow-hidden shadow-xl border border-[#000000]/10 flex flex-col h-full card-hover group stagger-2 relative">
                <div className="absolute top-4 left-4 z-20">
                  <span className="bg-[#01C9CF] text-black font-bebas text-xl px-4 py-1 uppercase tracking-wider rounded-lg shadow-md">
                    {t('raffle.prizes.secondPrizeLabel')}
                  </span>
                </div>
                <div className="flex flex-col md:flex-row h-full">
                  <div className="md:w-2/5 aspect-square md:aspect-auto overflow-hidden relative bg-[#000000]">
                    <img
                      ...
                      alt={t('raffle.prizes.secondPrizeTitle')}
              ...
              <div className="bg-white rounded-3xl overflow-hidden shadow-xl border border-[#000000]/10 flex flex-col h-full card-hover group stagger-3 relative">
                <div className="absolute top-4 left-4 z-20">
                  <span className="bg-[#EB7A4B] text-white font-bebas text-xl px-4 py-1 uppercase tracking-wider rounded-lg shadow-md">
                    {t('raffle.prizes.thirdPrizeLabel')}
                  </span>
                </div>
                <div className="flex flex-col md:flex-row h-full">
                  <div className="md:w-2/5 aspect-square md:aspect-auto overflow-hidden relative bg-[#000000]/5">
                    <img
                      ...
                      alt={t('raffle.prizes.thirdPrizeTitle')}
          ...
          <div className="mt-16 text-center fade-in-up stagger-3">
            ...
            >
              ...
              <span className="relative z-10 flex items-center gap-4">
                ...
                {isCheckoutLoading ? t('raffle.prizes.redirecting') : t('raffle.prizes.button')}
              </span>
            </button>
            {checkoutError && (
      ...
      <section className="py-24 md:py-32 bg-[#01C9CF] relative overflow-hidden text-center">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10 fade-in-up">
          ...
          >
            <iconify-icon icon="solar:ticket-bold-duotone" width="28" height="28"></iconify-icon>
            {isCheckoutLoading ? t('raffle.finalCta.redirecting') : t('raffle.finalCta.button')}
          </button>
          {checkoutError && (
